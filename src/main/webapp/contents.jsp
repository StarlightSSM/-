...
dao = new UserDAO();
dao.getUser(email);
userdto = new UserDAO().getUser(email);
if (!email.equals(userdto.getEmail())) {
	PrintWriter script = response.getWriter();
	script.println("<script>");
	script.println("alert('권한이 없습니다')");
							...
							<div class="fourth">
								<div class="maindiarybox">
									<div class="subdiarybox">
										...
										%>
										<div class="diarybox">
											<a class="select" href="selectAction.jsp?w_id=<%=list.get(i).getW_id()%>">선택하기</a>
											<div class="day" onclick="location.href='view.jsp?w_id=<%= list.get(i).getW_id() %>'">
												작성일자 :
												<%=list.get(i).getW_day().substring(0, 11) + list.get(i).getW_day().substring(11, 13) + "시"
		+ list.get(i).getW_day().substring(14, 16) + "분"%></div>
											<div class="diarycontents">
												제목 :
												<%=list.get(i).getW_title()%><br>
												<hr>
												작성자 :
												<%=list.get(i).getEmail()%>
											</div>
